...
new class extends Component
{
    ...
    {
        if ($this->selectAll) {
            $this->selectedComponents = $this->getComponentsQuery()->pluck('id')->all();
        } else {
    ...
    public function openEditModal(int $componentId): void
    {
        $post = Post::where('tenant_id', $this->tenantId)
            ->ofType(PostType::Component)
            ->findOrFail($componentId);

        $this->editingComponentId = $post->id;
    ...
    public function save(): void
    {
        $uniqueSlugRule = \Illuminate\Validation\Rule::unique('posts', 'slug')
            ...
            ->where('type_id', PostType::Component->value);

        if ($this->editingComponentId) {
            $uniqueSlugRule->ignore($this->editingComponentId);
        }
        ...
        $validated = $this->validate([
            ...
            'formTitle.required' => 'El nombre es obligatorio.',
            'formSlug.required' => 'El slug es obligatorio.',
            'formSlug.regex' => 'Solo letras minúsculas, números y guiones.',
            'formSlug.unique' => 'Ya existe un componente con este slug.',
        ]);
        ...
        if ($this->editingComponentId) {
            ...
            session()->flash('message', 'Componente actualizado exitosamente.');
        } else {
            Post::create([
            ...
            session()->flash('message', 'Componente creado exitosamente.');
        }
    ...
    public function updateStatus(int $componentId, string $status): void
    {
        $post = Post::where('tenant_id', $this->tenantId)
            ->ofType(PostType::Component)
            ->findOrFail($componentId);
    ...
    public function duplicateComponent(int $componentId): void
    {
        $post = Post::where('tenant_id', $this->tenantId)
            ->ofType(PostType::Component)
            ->findOrFail($componentId);
        ...
        session()->flash('message', 'Componente duplicado exitosamente.');
    }

    public function deleteComponent(int $componentId): void
    {
        Post::where('tenant_id', $this->tenantId)
            ->ofType(PostType::Component)
            ->where('id', $componentId)
            ->delete();

        $this->selectedComponents = array_values(array_diff($this->selectedComponents, [$componentId]));

        session()->flash('message', 'Componente eliminado exitosamente.');
    }

    public function applyBulkAction(): void
    {
        if (empty($this->bulkAction) || empty($this->selectedComponents)) {
            return;
        }
     ...
     */
    private function getComponentsQuery(): \Illuminate\Database\Eloquent\Builder
    {
        return Post::query()
            ->where('tenant_id', $this->tenantId)
            ->ofType(PostType::Component)
...
<div>
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="mb-8 flex items-center justify-between">
            <flux:heading size="xl">Componentes de Páginas</flux:heading>
                ...
                <div class="flex flex-col gap-4 sm:flex-row">
                    ...
                    <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                        @if ($components->total() > 0)
                            {{ $components->total() }} componente{{ $components->total() !== 1 ? 's' : '' }} encontrado{{ $components->total() !== 1 ? 's' : '' }}
                        @else
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center">
                            ...
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">
                                Seleccionar todo ({{ count($selectedComponents) }})
                            </span>
                        </div>
                    ...
                    <div class="flex items-center space-x-4">
                        <span class="text-sm text-gray-500 dark:text-gray-400">
                            {{ $components->count() }} elementos mostrados de {{ $components->total() }}
                        </span>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                        @forelse ($components as $postComponent)
                            <tr
                                wire:key="component-{{ $postComponent->id }}"
                                class="transition-colors duration-150 hover:bg-gray-50 dark:hover:bg-gray-700 {{ in_array($postComponent->id, $selectedComponents) ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}"
                            >
                                {{-- Checkbox --}}
                                <td class="whitespace-nowrap px-6 py-4">
                                    <flux:checkbox wire:model.live="selectedComponents" value="{{ $postComponent->id }}" />
                                </td>
                                ...
                                <td class="whitespace-nowrap px-6 py-4">
                                    <div class="flex flex-col">
                                        <div class="cursor-pointer text-sm font-medium text-gray-900 hover:text-blue-600 dark:text-white dark:hover:text-blue-400">
                                            <flux:icon.cube class="inline size-4" />
                                            {{ $postComponent->title }}
                                        </div>
                                        <div class="mt-1 flex items-center space-x-3 text-xs text-gray-500 dark:text-gray-400">
                                            <a href="{{ route('tenants.posts.page-builder', $postComponent->id) }}?slug={{ $postComponent->slug }}" target="_blank" class="hover:text-blue-600 dark:hover:text-blue-400">Editar componente</a>
                                            @if ($postComponent->getRawOriginal('status') !== 'archived')
                                                <span>|</span>
                                                <button wire:click="updateStatus({{ $postComponent->id }}, 'archived')" class="hover:text-red-600 dark:hover:text-red-400">Archivar</button>
                                            @endif
                                            @if ($postComponent->getRawOriginal('status') !== 'draft')
                                                <span>|</span>
                                                <button wire:click="updateStatus({{ $postComponent->id }}, 'draft')" class="hover:text-yellow-600 dark:hover:text-yellow-400">Borrador</button>
                                            @endif
                                            @if ($postComponent->getRawOriginal('status') !== 'published')
                                                <span>|</span>
                                                <button wire:click="updateStatus({{ $postComponent->id }}, 'published')" class="hover:text-green-600 dark:hover:text-green-400">Publicar</button>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <div class="select-all rounded border border-gray-200 bg-gray-100 px-2 py-1 font-mono text-xs text-gray-600 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-400">
                                            renderComponent('{{ $postComponent->slug }}')
                                        </div>
                                        <button
                                            x-data
                                            @click="navigator.clipboard.writeText('renderComponent(\'' + '{{ $postComponent->slug }}' + '\')')"
                                            class="ml-1 rounded p-1 text-gray-400 transition-colors duration-150 hover:bg-blue-100 hover:text-blue-600 dark:hover:bg-blue-900 dark:hover:text-blue-400"
                                ...
                                {{-- Status --}}
                                <td class="whitespace-nowrap px-6 py-4">
                                    @php $rawStatus = $postComponent->getRawOriginal('status'); @endphp
                                    <span @class([
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col">
                                        <span>{{ $postComponent->created_at->format('d/m/Y') }}</span>
                                        <span class="text-xs text-gray-400 dark:text-gray-500">{{ $postComponent->created_at->format('H:i') }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <flux:button variant="ghost" size="sm" icon="pencil-square" wire:click="openEditModal({{ $postComponent->id }})" title="Editar" />
                                        <flux:button variant="ghost" size="sm" icon="document-duplicate" wire:click="duplicateComponent({{ $postComponent->id }})" title="Duplicar" class="!text-green-600 hover:!text-green-900 dark:!text-green-400" />
                                        <flux:button variant="ghost" size="sm" icon="trash" wire:click="deleteComponent({{ $postComponent->id }})" wire:confirm="¿Estás seguro de eliminar este componente?" title="Eliminar" class="!text-red-600 hover:!text-red-900 dark:!text-red-400" />
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="mx-auto mb-4 h-24 w-24 text-gray-400 dark:text-gray-500">
                                        <flux:icon.cube class="mx-auto size-16" />
                                    </div>
                                    <h3 class="mb-2 text-lg font-medium text-gray-900 dark:text-white">No hay componentes creados</h3>
                                    <p class="mb-6 text-gray-500 dark:text-gray-400">Comienza creando tu primer componente de página</p>
                                    <flux:button variant="primary" wire:click="openCreateModal" icon="plus">
    ...
    <flux:modal wire:model.self="showModal" class="md:w-96">
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ $editingComponentId ? 'Actualizar Componente' : 'Crear Componente' }}</flux:heading>
            </div>

            <flux:field>
                <flux:label>Nombre del Componente</flux:label>
                <flux:input wire:model="formTitle" placeholder="Ingresa el nombre del componente" />
                <flux:error name="formTitle" />
            </flux:field>

            <flux:field>
                <flux:label>Slug</flux:label>
                <flux:input wire:model="formSlug" placeholder="ej-header-footer-slider" class="font-mono text-sm" />
            ...
            <div class="flex justify-end gap-3">
                ...
                <flux:button type="submit" variant="primary" icon="check">
                    {{ $editingComponentId ? 'Actualizar' : 'Crear' }}
                </flux:button>
            </div>
        </form>
    </flux:modal>
